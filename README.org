...
- =daemon/= :: Cargo workspace with eight crates: =eav-core= (wire types),
  ...
  HTTP + SSE), =eav-mcp= (optional Model Context Protocol scaffold), and
  the =eavd= binary that ties them together.
...
\\* The MCP server is a scaffold in v0.4.0 — resource/tool listing works,
the JSON-RPC stdio loop returns =bail!("not implemented")=. Will be
finished in v1.0.
